...
class App extends Component {
  ...
  updateBook() {
    ...
    axios.put('http://localhost:3000/books/' + this.state.editBookData.id, {
      ...
      this.setState({
        editBookModal: false, editBookData: { id: '', title: '', ISBN: '' }
      })
    });
  }
  editBook(id, title, ISBN) {
    this.setState({
      editBookData: { id, title, ISBN }, editBookModal: ! this.state.editBookModal
    });
  }
  deleteBook(id) {
    ...
    let books = this.state.books.map((book) => {
      return (
        <tr key={book.id}>
          ...
          <td>{book.ISBN}</td>
          <td>
            <Button color="success" size="sm" className="mr-2" onClick={this.editBook.bind(this, book.id, book.title, book.ISBN)}>Edit</Button>
      ...
      <Modal isOpen={this.state.newBookModal} toggle={this.toggleNewBookModal.bind(this)}>
        ...
        <ModalBody>
          ...
          <FormGroup>
            ...
            <Input id="ISBN" value={this.state.newBookData.rating} onChange={(e) => {
                let { newBookData } = this.state;
        ...
        <ModalBody>
          ...
          <FormGroup>
            <Label for="ISBN">ISBN</Label>
              <Input id="ISBN" value={this.state.editBookData.rating} onChange={(e) => {
              let { editBookData } = this.state;
